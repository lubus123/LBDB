...
export function createAppServer() {
          ...
          if (user) {
            for (const [oldWs, player] of room.players) {
              if (player.userId === user.id && oldWs !== ws) {
                ...
                log.info(`[${msg.gameId}] ${user.username} reconnected`);
                return;
              }
            }
          }
  ...
  return { httpServer, wss, rooms, playerRooms, authenticatedUsers, wsUserMap, pendingChallenges };
}

// ─── Startup (skip when imported by tests) ───
...
if (!isTest) {
  (async function start() {
    ...
    httpServer.listen(PORT, () => {
      log.info(`duckGammon server listening on port ${PORT}`);
      if (SERVE_STATIC) log.info(`Serving static files from ${DIST}`);
    });
  })().catch(err => {
    ...
    const { httpServer } = createAppServer();
    httpServer.listen(PORT, () => {
      log.info(`duckGammon server listening on port ${PORT} (no database)`);
    });
  });
}
